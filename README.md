...
O repositório inclui uma configuração para **Cloudflare Containers**, que exige o plano Workers Paid. O container executa Python, Node.js e FFmpeg, e o Worker encaminha as requisições. Para publicar, ative Containers na conta, instale as dependências JavaScript com `pnpm install` e execute `pnpm run deploy` com Docker ativo, ou conecte o repositório ao Workers Builds com o comando de implantação `pnpm exec wrangler deploy`. Teste os downloads no endereço gerado antes de compartilhá-lo. Downloads públicos exigem limites de acesso e monitoramento de custos; esta configuração mantém no máximo uma instância ativa.
